...
# ==========================================
# 0. FIX ALL RANDOM SEEDS FOR REPRODUCIBILITY
# ==========================================
SEED = 42
...
# ==========================================
train_val_df, test_df = train_test_split(df, test_size=0.15, random_state=SEED, stratify=df["disease_id"])
train_df, val_df = train_test_split(train_val_df, test_size=0.1, random_state=SEED, stratify=train_val_df["disease_id"])
...
train_df_balanced = pd.concat([resample(l, replace=True, n_samples=max_size, random_state=SEED) for l in lst])
...
# PyTorch DataLoader with reproducible seed worker
...
train_loader = DataLoader(MiRNADataset(train_ids, train_masks, train_bin, train_dis), batch_size=16, shuffle=True, worker_init_fn=seed_worker, generator=g)
val_loader = DataLoader(MiRNADataset(val_ids, val_masks, val_bin, val_dis), batch_size=16, shuffle=False)
test_loader = DataLoader(MiRNADataset(test_ids, test_masks, test_bin, test_dis), batch_size=16, shuffle=False)
...
# 4. t-SNE Clustering (Explicit random_state=SEED)
